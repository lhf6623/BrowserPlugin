  ...
  return (
    <div className='w-280px p-1 text-12px'>
      ...
      <footer className='w-full overflow-hidden relative text-right mt-1 *:text-lg *:opacity-30 *:text-#409eff all:transition-400'>
        <span className='absolute inline-block flex-center left-1 !text-#000 !text-12px'>v: {packageJson.version}</span>
        <a
          ...
          title='源代码'
        ></a>
        <button className='i-mdi:calendar-add hover:opacity-100' title='添加任务' onClick={handleShowPanel}></button>
        <button className='i-mdi:mixer-settings hover:opacity-100' title='任务面板配置' onClick={goOptions}></button>
      </footer>
    </div>
  );
}
