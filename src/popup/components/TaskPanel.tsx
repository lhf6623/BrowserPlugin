...
export default function AddTaskPanel({ show, title, task, onChange }: AddTaskPanelProps) {
  ...
  function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const start = dateUtils(formData.start).valueOf();
    const end = !endType
      ? dateUtils(formData.end).valueOf()
...
function EndTime({ task }: { task: Task }) {
  ...
  let defaultEnd: number | string = dateUtils(task.end).format("YYYY-MM-DD HH:mm");
  if (!isDatetime) {
    defaultEnd = dateUtils(task.end).diff(dateUtils(task.start), timetype, true);
  }
